...
在深入学习有根树之前，我们需要了解一些基本术语：

- **节点（Node）**：树中的基本元素，包含数据和指向其他节点的指针
- **根节点（Root）**：树的顶层节点，没有父节点
- **叶子节点（Leaf）**：没有子节点的节点
- **内部节点（Internal Node）**：至少有一个子节点的节点
- **父节点（Parent）**：对于任意非根节点，直接连接到它的上层节点
- **子节点（Child）**：对于任意节点，直接连接到它的下层节点
- **兄弟节点（Sibling）**：具有相同父节点的节点
- **祖先节点（Ancestor）**：从根节点到该节点路径上的所有节点
- **子孙节点（Descendant）**：以该节点为根的子树中的所有节点
...
SEARCH-NODE(x, key):
    if x = NIL or x.key = key
        return x
    result = SEARCH-NODE(x.left, key)
    if result ≠ NIL
        return result
    return SEARCH-NODE(x.right, key)
...
SEARCH-NODE(x, key):
    ...
    if x.key = key
        return x
    result = SEARCH-NODE(x.left-child, key)
    if result ≠ NIL
        return result
    return SEARCH-NODE(x.right-sibling, key)
```
