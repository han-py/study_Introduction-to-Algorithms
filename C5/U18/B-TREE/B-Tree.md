...
让我们通过一个最小度数为3的B树（也就是2-3-4树）来直观理解这些性质：
...
```
      ...
      [5 7]  [15] [25]  [35 40]
      / | \   / \  / \   /  |  \
   [2][6][8][12][18][22][27][38][45]
```

在这个例子中：
- 最小度数t=3，所以每个节点最少有2个关键字，最多有5个关键字
- 根节点[10 20 30]有3个关键字，4个子节点
- 内部节点[5 7]有2个关键字，3个子节点
- 所有叶节点[2][6][8][12][18][22][27][38][45]都在同一层
- 关键字在每个节点内按升序排列
...
例如，对于一个包含10亿个关键字的B树（n=10^9），当t=100时：
```
h ≤ log_100((10^9+1)/2) ≈ log_100(5×10^8) ≈ 4.35
```

这意味着即使有10亿个关键字，我们也只需要最多5次磁盘访问就能找到任何一个关键字！
...
**插入30**（节点满，需要分裂）：
步骤1：创建新节点，将后半部分关键字移到新节点
```
[10 20 30]  =>  [10 20] 和 [30]
```

步骤2：将中间关键字20上移到新的根节点
```
    [20]
   /    \
[10]    [30]
